...
public class TouchController3 : MonoBehaviour
{
    ...
    {
        InputManagerGeneral.Instance.SetTouchWheel(horizontalDelta/maxDeltaRangeLeftTouch);
    }
    ...
    [SerializeField]
    [Tooltip("Angle (in degrees) within which steering will snap back to exactly zero.")]
    private float steeringSnapAngle = 0.01f;

    [SerializeField]
    // [Tooltip("Delta value within which horizontalDelta will snap back to exactly zero.")]
    private float horizontalSnapThreshold = 0.01f;

    private void ReturnSteeringToCenter()
    {
        if (IsControlling) return;

        // 1) Rotate smoothly toward zero
        Quaternion currentRot = stearImage.transform.localRotation;
        ...
        // 2) If we're within snap‐angle, just set to identity
        if (Quaternion.Angle(currentRot, targetRot) <= steeringSnapAngle)
        {
            stearImage.transform.localRotation = targetRot;
        }
        else
        {
            stearImage.transform.localRotation = newRot;
        }

        // 3) Ease horizontalDelta back to zero
        float newDelta = Mathf.MoveTowards(
            horizontalDelta,
            0f,
            (maxDeltaRangeLeftTouch / maxStearingAngle) * steeringReturnSpeed * Time.deltaTime
        );
        
        // 4) If it's within snap threshold, zero it out
        if (Mathf.Abs(newDelta) <= horizontalSnapThreshold)
        {
            horizontalDelta = 0f;
        }
        else
        {
            horizontalDelta = newDelta;
        }
    }
